...
TMXTileLayer* TMXLoader::getTMXTileLayer(std::string mapName, std::string layerName)
{
	// Attempt to print data for a specific map
	...
	{
		return iterator->second->getTileLayer(layerName);
	}
	return nullptr;
}
        ...
        while (currentNode != nullptr)
        {
            if (currentTile < layerWidth)
            {
				...
				if(currentNode->first_attribute() == nullptr)
					tileVector[currentRow][currentTile] = (unsigned int)0;
				else
